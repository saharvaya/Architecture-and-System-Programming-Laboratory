...
int main (int argc , char* argv[], char* envp[])
{
    ...
    cmdLine* cmdl;
    int debugFlag=0,i,status,signal;
    pid_t PID;
     for(i=1;i<argc;i++){
    ...
    while(1){
        ...
        if(strcmp(input,"procs\n")==0)
            ...
            if(strcmp(cmdl->arguments[0],"kill")==0){
                kill(cmdl->arguments[1],SIGINT);
                updateProcessStatus(dummy,(int)cmdl->arguments[1],-1);
                
            }
            if(strcmp(cmdl->arguments[0],"suspend")==0){
                kill(cmdl->arguments[1],SIGTSTP);
                updateProcessStatus(dummy,(int)cmdl->arguments[1],0);
            }
            if(strcmp(cmdl->arguments[0],"wake")==0){
                kill(cmdl->arguments[1],SIGCONT);
                updateProcessStatus(dummy,(int)cmdl->arguments[1],1);
            }
            if(debugFlag==1){
