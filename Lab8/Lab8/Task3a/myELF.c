...
struct fun_desc menu[]={{"Toggle Debug Mode",&toggle}, {"Examine ELF File", &Examine_ELF},{"Print Section Names",&print_sections},{"Print Symbols",&print_symbols},{"Relocation Tables",&relocate_tables}, {"Quit", &quit}, {NULL,NULL}};
    ...
    if(Currentfd==-1)
        ...
        for( i = 0; i < header->e_shnum; i++){
            if(section->sh_type==SHT_SYMTAB || section->sh_type==SHT_DYNSYM){
                ...
                symbol_string_table = map_start + sectionheader[section->sh_link].sh_offset;
